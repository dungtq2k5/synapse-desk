...
/**
 * The e2e layer at notification-service's own boundary: real Prisma against a
 * real Postgres (the TEST database — see .env.test), real service wiring, no
 * mocked repository.
 *
...
const config: Config = {
  ...
  // Real Postgres round trips. The 5s jest default is tight for a suite that
  // truncates and re-inserts between tests.
  testTimeout: 30_000,
};
