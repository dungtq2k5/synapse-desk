...
export class SchedulerRegistrar implements OnApplicationBootstrap {
  ...
  constructor(
    @InjectQueue(SCHEDULER_QUEUE.auth) private readonly queue: Queue,
  ) {}

  async onApplicationBootstrap(): Promise<void> {
    await this.register(SCHEDULED_JOBS.AUTH_HOURLY);
    await this.register(SCHEDULED_JOBS.AUTH_DAILY);
  }
