...
| **citations** | JSONB | Nullable | What an AI answer cited — an array of objects with **exactly five keys**, whichever path wrote the row: `chunkId`, `documentId`, `documentTitle`, `pageNumber` (null for a format with no pages) and `vectorPointId`. Written by both AI-answer paths — the streamed chat answer and the auto-reply — so citations survive a reload instead of living only in the `ai:stream:done` frame. **`NULL` and `[]` are different answers**: NULL is a human message, or an AI row written before citations were stored; `[]` is an answer that cited nothing. A column rather than a child table because nothing queries citations on their own — the thread wants them *with* the message, and "which answers cited chunk X" is already `ai_generations.cited_chunk_ids` (Table 29). |
