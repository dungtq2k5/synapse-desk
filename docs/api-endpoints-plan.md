...
| PUT | `/roles/:id/permissions` ✎ | Replace the permission set in one transaction. Provenance to `audit_logs` (`ROLE_PERMISSIONS_CHANGED`); `_role_permissions` holds no payload columns (RDM Tables 7–8). Subject to the same no-escalation rule as role assignment: an actor may not grant a permission they do not themselves hold. | perm:`role.permission.assign` |
| GET | `/permissions` | Full permission catalogue, grouped by `target` prefix — drives the role editor UI. | perm:`role.read` |
| GET | `/roles/:id/users` | Users holding the role. **NOT BUILT.** | perm:`role.read` |
| POST | `/roles/:id/users` ✎ | Bulk-assign the role to users. **NOT BUILT** — reachable today via `PUT /users/:id/roles` per user. | perm:`user.role.assign` |
| DELETE | `/roles/:id/users/:userId` ✎ | Revoke the role from a user. **NOT BUILT** — same. | perm:`user.role.assign` |
...
| GET | `/platform/permissions` | Permission registry. **NOT BUILT** — `GET /permissions` already serves the catalogue to any tenant with `role.read`, so a Super Admin view adds nothing until permissions become mutable. | SUPER |
...
| PATCH | `/tickets/:id` ✎ | Update `title`, `description`, `priority`. | perm:`ticket.update` |
...
| POST | `/tickets/:id/status` ✎ | State-machine transition over `NEW → OPEN → PENDING_AGENT → ESCALATED → RESOLVED → CLOSED`. Stamps `resolved_at`. Illegal transitions ⇒ **409**. | perm:`ticket.update` |
| POST | `/tickets/:id/priority` ✎ | Change `priority` (`LOW\|MEDIUM\|HIGH\|URGENT`). | perm:`ticket.update` |
...
| POST | `/tickets/:id/escalate` ✎ | Tier 1 → Tier 2. Sets `status = ESCALATED` + `escalated_at`, routes to a department, and triggers AI summary generation (RDM §1.3). One-click escalation from chat. | USER |
| POST | `/tickets/:id/resolve` ✎ | Mark resolved (+ optional resolution note as a message). | perm:`ticket.resolve` |
| POST | `/tickets/:id/reopen` ✎ | `RESOLVED`/`CLOSED` → `OPEN`, clears `resolved_at`. | USER |
| POST | `/tickets/:id/close` ✎ | Final close. | perm:`ticket.update` |
