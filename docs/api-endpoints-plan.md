...
| GET | `/tickets/:id/history` | **STATUS history**, oldest first, from `ticket_status_changes` — written inside the status transaction, so it has no holes. Not from `audit_logs`, which is at-most-once and read behind an admin permission ([ADR 0040](./decisions/0040-ticket-status-history-is-a-table-not-a-trail.md)). Reassignments are at `GET /tickets/:id/assignments`; merging the two would be one list with two shapes. Unpaginated. | USER |
