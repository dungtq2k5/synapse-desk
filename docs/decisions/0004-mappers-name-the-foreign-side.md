...
### The prefixes look asymmetric and are not
...
### The directions behave differently
...
**One documented exception.** `toIsoDate(date: Date): string` is named for its *format*, not its return type — `toString` would be meaningless and would collide with the builtin. **When the return type's name carries no information, name the thing that does.**
