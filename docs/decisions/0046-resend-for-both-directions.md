...
Inbound attachments are **dropped, by name, in release 1** — the filenames
reach the ticket's note as they did when storage failed — and return in
release 2 through a storage-service RPC that fetches them server-side.
...
- **The threading fallback is the one thing a provider swap can silently
  ...
  rather than assumed.
- **Attachments are not carried across in one release, deliberately.** The
  Worker put attachment bytes in the bucket without touching an application
  server — the property the presign flow exists to hold. Resend hands bytes
  back behind a signed URL, and the only shape that keeps the property is a
  storage-service fetch that does not exist yet. Streaming bytes through the
  gateway to make release 1 feature-complete would trade a defended
...
- ADR 0018's rules about routing and threading stand. Its bullets about the
  ...
  fixture contract test as its only check — are superseded; its _"No
  attachments via inbound email"_ is true again until release 2.
