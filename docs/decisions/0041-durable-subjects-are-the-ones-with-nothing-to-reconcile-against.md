...
| `document.*` | Doc 49's sweep reconciles from state, within ten minutes |
...
- **A poison message is terminated, and terminated messages go somewhere.** JetStream has no built-in dead-letter, so `MaxDeliver` is followed by a republish to a `.dlq` subject before `term()`. This is doc 40 §3 and doc 49 §2 arriving from a third direction: a deterministic failure retried forever blocks everything behind it. `MaxDeliver` is the JetStream spelling of the `try`/`catch` doc 49 added to the sweep.
...
- **Doc 47 §1's choice of a `ticket_status_changes` table over consuming `ticket.status_changed` (ADR 0040) survives unchanged.** `ticket.*` is still at-most-once, so its reasoning still holds — and the permission boundary settled it independently anyway.
