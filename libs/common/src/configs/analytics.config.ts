 ...
 *
 * **The name is historical and the scope is not.** This began as rollup-only
 * and now spans live-row exports too — `TICKET` and `AUDIT_LOG` read `tickets`
 * and `audit_logs`, neither of which is analytics. The lifecycle is what the
 * three share: one status machine, one queue, one processor, one poll route,
 * one table.
 *
 * Renaming the family to `Export` was considered and **deliberately deferred**:
 * it is ~18 files plus a proto regeneration and the enum-bridge registry, and
 * it buys readability rather than behaviour. Recorded in `known-gaps.md` rather
 * than left for a reader to infer from a table called `analytics_exports`
 * holding audit-log rows.
 */
...
] as const satisfies readonly AnalyticsExportKind[];
