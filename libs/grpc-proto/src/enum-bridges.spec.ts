...
describe('every enum bridge round-trips', () => {
  ...
  const bridges: Bridge[] = [
    ...
    [
      'SearchDegradation',
      Object.values(RetrievalDegradation),
      toProtoSearchDegradation,
      fromProtoSearchDegradation,
    ],
    [
    ...
    // Migrated off their hand-written form, which is why they arrive here as a
    // group. They predated `enumBridge` by two weeks and were covered only by
    // `mappers.spec.ts`; the exhaustive `Record<D, P>` now catches a member
    // added on one side and forgotten on the other, which nothing did before.
    [
      'InvitationStatus',
    ...
    [
      'OrgStatus',
      Object.values(OrgStatus),
      toProtoOrgStatus,
      fromProtoOrgStatus,
    ],
    ['AiModelTier', AI_MODEL_TIERS, toProtoAiModelTier, fromProtoAiModelTier],
    [
      'NotificationPriority',
